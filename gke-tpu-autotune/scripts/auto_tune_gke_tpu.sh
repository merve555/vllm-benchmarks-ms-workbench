...
# Adapted for GKE with TPU environment.
...
start_server() {
    ...
    # TPU-specific vLLM serve command
...
run_benchmark() {
    ...
    python -m vllm.entrypoints.benchmark.serve \
        --backend vllm \
        --model $MODEL  \
        --dataset-name random \
...
# For TPU, we'll use a fixed gpu_memory_utilization since TPU memory management is different
gpu_memory_utilization=0.95
echo "Using gpu_memory_utilization=$gpu_memory_utilization for TPU."
...
# Copy results to a shared volume for access from outside the pod
cp -r "$LOG_FOLDER" /shared/autotune-results/ 2>/dev/null || echo "Warning: Could not copy results to shared volume"
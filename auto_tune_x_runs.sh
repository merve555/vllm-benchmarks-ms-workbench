...
NUM_BENCHMARK_RUNS=4 # Number of times to run the benchmark for each combination
...
run_benchmark() {
    ...
    for i in $(seq 1 $NUM_BENCHMARK_RUNS); do
        echo "Running benchmark test (Run $i / $NUM_BENCHMARK_RUNS)..."
        meet_latency_requirement=0
        bm_log="$LOG_FOLDER/bm_log_${max_num_seqs}_${max_num_batched_tokens}_run_${i}_requestrate_inf.txt"
        ...
        # start from request-rate as int(throughput) + 1
            ...
            while ((request_rate > 0)); do
                ...
                sleep 5
                bm_log="$LOG_FOLDER/bm_log_${max_num_seqs}_${max_num_batched_tokens}_run_${i}_requestrate_${request_rate}.txt"
                python benchmarks/benchmark_serving.py \
                    --backend vllm \
                    --model $MODEL \
                    --dataset-name random \
        ...
        if ((meet_latency_requirement)); then
            echo "Run $i: max_num_seqs: $max_num_seqs, max_num_batched_tokens: $max_num_batched_tokens, request_rate: $request_rate, e2el: $e2el, throughput: $throughput, goodput: $goodput"
            echo "Run $i: max_num_seqs: $max_num_seqs, max_num_batched_tokens: $max_num_batched_tokens, request_rate: $request_rate, e2el: $e2el, throughput: $throughput, goodput: $goodput" >> "$RESULT"
            ...
            echo "Run $i: max_num_seqs: $max_num_seqs, max_num_batched_tokens: $max_num_batched_tokens does not meet latency requirement ${MAX_LATENCY_ALLOWED_MS}"
            echo "Run $i: max_num_seqs: $max_num_seqs, max_num_batched_tokens: $max_num_batched_tokens does not meet latency requirement ${MAX_LATENCY_ALLOWED_MS}" >> "$RESULT"
        fi
        echo "Current best throughput so far: $best_throughput"
        echo
    done
...
echo "best_max_num_seqs: $best_max_num_seqs, best_num_batched_tokens: $best_num_batched_tokens, best_throughput: $best_throughput"
echo "best_max_num_seqs: $best_max_num_seqs, best_num_batched_tokens: $best_num_batched_tokens, best_throughput: $best_throughput, best_goodput: $best_goodput" >> "$RESULT"